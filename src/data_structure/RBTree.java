 ...
 * 红黑树
 * 
 * 原理： http://www.cnblogs.com/skywang12345/p/3245399.html
 * 
 * java实现： http://www.cnblogs.com/skywang12345/p/3624343.html
 * 
 * @author han
 *
 * @param <T>
...
public class RBTree<T extends Comparable<T>> {
	...
	/**
	 * 查找节点x的后继节点。即查找红黑树中数据值大于该节点的最小节点
	 * 
	 * @param x
	 * @return
	 */
	...
	private void insert(RBTNode<T> node) {
		...
		// 1.将红黑树当做一颗二叉查找树，将节点天价到二叉查找树中
		while (x != null) {
			y = x;
			cmp = node.key.compareTo(x.key);// 比较插入点与根值大小
			if (cmp < 0) {
				// node值小于根值，继续比较红黑树左节点
				x = x.left;
			} else {
				x = x.right;
			}
	 ...
	 * 红黑树插入修正函数
	 * 
	 * 红黑树插入节点后，失去平衡，再调用该函数，目的是将它重新塑造成一颗红黑树
	 * 
	 * @param node
	 */
	private void insertFixUp(RBTNode<T> node) {
		...
		while (((parent = parentOf(node)) != null) && isRed(parent)) {
			...
			if (parent == gparent.left) {
				RBTNode<T> uncle = gparent.right;// 叔叔节点
				// 条件1，叔叔节点是红色
				...
				// 父节点是祖父节点右孩子
				RBTNode<T> uncle = gparent.left;// 叔叔节点
				if ((uncle != null) && isRed(uncle)) {
					setBlack(uncle);
					setBlack(parent);
					setRed(gparent);
